...
$(document).ready(function () {
    $("#user-content li").click(function (e) {

        var className = $(e.target).attr('id');
        if (className == "arrow1") {
            $("#user-content .tab1").animate({ width: "64px" }, 300).removeClass("active");
            $("#user-content .tab2").animate({ width: "780px" }, 300).addClass("active");
            return false;
        } else if (className == "arrow2") {
            $("#user-content .tab2").animate({ width: "64px" }, 300).removeClass("active");
            $("#user-content .tab3").animate({ width: "780px" }, 300).addClass("active");
            return false;
        }
        ...
        if (!$(this).hasClass("active")) {
            ...
            for (var i = 0; i < 3; i++) {
                if ($("#user-content li").eq(i).hasClass("active")) {
                    $("#user-content li").eq(i).animate({ width: "64px" }, 300).removeClass("active");
                }
            }
